...
如果使用 `--pthreads` 构建，则线程版产物会写到 `dist/<ver>/pthreads/`，并保持同样的
`mujoco.js` / `mujoco.wasm` 文件名。
...
- `exports_check.json` / `exports_check.pthreads.json` / `exports_report_funcs.md`：导出检查结果与可读报告。
...
运行时 checks 也支持选择变体目录：

- `MJWF_DIST_VARIANT=pthreads`（或 `DIST_VARIANT=pthreads`）
