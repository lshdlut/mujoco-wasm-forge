...
const distDir = path.resolve(rootDir, "dist", "3.3.8-alpha");
const wasmPath = path.join(distDir, "mujoco.wasm");
const manifestPath = path.join(distDir, "abi", "wrapper_exports.json");

assert.ok(fs.existsSync(wasmPath), "dist/3.3.8-alpha/mujoco.wasm missing");
assert.ok(fs.existsSync(manifestPath), "dist/3.3.8-alpha/abi/wrapper_exports.json missing");
...
console.log(
  `regression(3.3.8-alpha): exports ok (${required.size} required, optional miss=${missingOptional.length})`,
);
